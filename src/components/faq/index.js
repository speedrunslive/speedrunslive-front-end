...
const FAQBody = () => {
 ...
 <div className="row">
    ...
    <div className="pull-left col-md-9" id="faq">
        ...
        <p>People speedrun to challenge themselves, to see a game pushed to the limits, and to get extra replay value out of
            a game.</p>
        ...
        <p>SRL Races are currently the most developed facet of SpeedRunsLive. Races let you run in direct competition with others
            simultaneously. Players set up races, and the RaceBot gives them a countdown. On GO! they start at the same time
            and both play through the game with the intention of finishing the goal first. Races are livestreamed so others
            can watch the action.</p>
        <p>Every game raced on SRL has a ranking leaderboard. Players may rise to a higher rank when they do well in races.
            The stronger the opponent, the more you have to gain. The weaker the opponent, the more you have to lose. Most
            of all, we're all in this to have a good time. While races have potential for really great competition, it's
            all about fun.</p>
        ...
        <p>
            We also offer an embedded IRC client on our site <Link to="/channel/">here</Link>. However, the mibbit 
            client lacks many features that other IRC clients have, so we heavily suggest
            using a full IRC client such as those mentioned above.
        </p>
        ...
        <p>We'd also suggest using the <code>/list</code> command, which lists the many channels which make up the SRL IRC.
            These range from game-specific channels to general subgroups of our large community.</p>
        <h3 id="getregged">4. Register your nick</h3>
        <ol>
            <li>Sign on to the nick you want to use for all future races, and type <code>/nickserv register <em>password e-mail</em></code>
                . It is important that you enter your actual e-mail address, in case you need to retrieve your password
                in the future.</li>
            ...
            <li>(optional) Set up automatic login within your IRC client. You may do this in mIRC by going to Options, then the
                Connect menu, then the Options menu. Click Perform, and type <code>/nickserv identify <em>password</em></code>.
                mIRC will automatically enter your password when you join the SRL IRC in the future.</li>
        </ol>
        <p>Advanced users may want to try our custom-built script, provided <a href="http://pastebin.com/kiVCE5Fh">here</a>.
            Be sure to substitute your own login information.
        </p>
        <p>
        ...
        <p>
            Type <code>.setstream twitch/<em>name</em></code> in the <strong>#speedrunslive</strong> channel to link your
            stream to SRL.
        </p>
        <p>Additional information:</p>
        <ul>
            <li>You only need to do this once. You do not need to do it every time you log in or race.</li>
            <li>Your stream will appear on the Streams page of the site after 1 total hour of racing. You will need to visit
                the website while logged in once per month to stay on. Alternatively you can join the <strong>#speedrunslive</strong>                channel on IRC while identified at least once a month.
            </li>
            ...
            <li>In your Twitch settings, turn on <em>Automatically archive my broadcasts</em> in the Channel & Videos section.
                This will allow our moderators to check your VODs after the race if necessary.
            </li>
        </ul>
        ...
        <ul>
            <li>Join our IRC network as described above, and type <code>/list</code>. Browse through the list of IRC channels
                which comes up, and see if any pertain to your game or its series. Joining the community for a particular
                game or series is often the best first step towards getting more involved and learning more about it.</li>
            <li>Visit the <a href="http://forum.speeddemosarchive.com">SDA forums</a> and see if there is an existing thread
                about your game.</li>
            <li>There may be an existing site out there devoted entirely to providing strategies, routes, and/or leaderboards
                for your game or its series. Do a quick Google search and see what comes up!</li>
        </ul>
        <div className="faqdivider"></div>
        <h1 id="racing">Racing</h1>
        <h2 id="whatrace">What can I race on SpeedRunsLive?</h2>
        <p>We strongly encourage users to race goals that are meaningful and give time enough for the racers to demonstrate
            their levels of skill. Generally, any goal to beat the full game under any restriction is ok, as long as it exceeds
            approximately 3 minutes or more. If you want to race an extremely short game such as Tower of Heaven, one suggestion
            is to have the goal be to beat it several times in a row.</p>
        <p>Generally, any goal that only completes a part of the game, such as "Beat World 1", should be kept to at least about
            10 minutes of actual playtime. However, some race goals can exceed this length and still be discouraged to race
            (Example: Beat Brock in Pokemon R/B/Y. May exceed 10 minutes, but it's too short in respect to the rest of the
            game to offer any real measure of competition.). The same applies for an arbitrary goal such as "Get 1000 points"
            or "collect 100 such and suches". This is because we would like SRL to represent one's skill at the full game,
            so racing too many short goals can prevent the leaderboards from reflecting this.</p>
        ...
        <p>In order to race a game there needs to be a start point and an end point. The end point is whatever the goal is.
            The simplest end point is "any%" which means simply to beat the game. The term comes from the Metroid community,
            where people would do 100% runs, low% runs (beating the game with the least amount of items), and any% (anything
            goes).
        </p>
        <p>Some games don't really fit into percentages, such as Super Mario 64. Instead of percentage, the star count makes
            more sense to use. In Super Mario 64, a "120 star" goal would be to start a new game, collect all 120 stars,
            and beat the game.</p>
        ...
        <p>If the emulator is accurate and the community for that game accepts emulator as a valid choice, then yes, you can.
            The most commonly used emulators for each system include:</p>
        <ul>
        ...
        <p>It is strongly preferred that you stream for racing. If you are asked for proof, you must figure out a way to show
            proof of completion of the goal. Otherwise, you will be disqualified (and possibly banned). If you are playing
            on an emulator, or if you are racing a <Link to="/races/#!/seasons/1">season goal</Link>, you are required
             to stream (NO exceptions). However, if you are playing on a console, you
            are not required to stream. For more details about streaming and proofcalling, read below.</p>
        ...
        <h3>Streaming</h3>
        <p><strong>Consoles:</strong> Consoles can be streamed using a capture card to directly capture the console feed, or
            using a web camera pointed at your tv-screen. You are not required to stream if you are playing on a console,
            you can however still be proofcalled should someone doubt your time.</p>
        <p><strong>Emulators:</strong> All use of emulators MUST be streamed when racing. This is a requirement and there are
            no general exceptions. Certain emulators may be banned. If you happen to use an emulator that is running on an
            Android phone or other such device, you may not race until you have gotten permission from AT LEAST one operator
            in <strong>#speedrunslive</strong>. If you are doubting whether or not your emulator is affected by this rule,
            then assume that it is, and do not race without getting such permission. You can still be proofcalled in the
            event that you get permission to race without streaming, should someone doubt your time.</p>
        <p><strong>PC Games:</strong> All PC games MUST be streamed when racing. This is a requirement and there are no general
            exceptions. If you happen to believe that you cannot race while streaming because your game lags too much or
            you do not have sufficient bandwidth to stream, then you may not race until you have gotten permission from AT
            LEAST one operator in <strong>#speedrunslive</strong>. Also note that this permission will not be given for Minecraft,
            Minesweeper, Windows Desktop Games or other low requirement PC games that can be streamed on any computer purchased
            within the last four or so years. You can still be proofcalled in the event that you get permission to race without
        ...
        <h3>Proofcalling</h3>
        <p>Proofcalling is the way that we deal with cheaters and doubtful times on SpeedRunsLive. If a racer finishes with
            a time that seems to be above their skill level, or if a new racer suddenly gets a high ranking time on the race
            leaderboards, it is common to initiate a proofcall if that racer did not stream. Proofcalling can be done by
            anyone, whether in the race or not, and anyone can be proofcalled regardless of position. It is common to ask
            for assistance in <strong>#speedrunslive</strong> from a voice or operator to come handle the situation.</p>
        ...
        <p>These examples are general and it is unfortunately impossible to make a perfect guideline for what is sufficient
            proof, which is why we recommend streaming.</p>
        <p>Should you happen to be proofcalled and unable to provide proof for your time, as mentioned previously it does not
            necessarily label you a cheater, but rather unable to prove your time. You will be disqualified from the race,
            and in the worst cases where we do believe cheating has taken place, you will be banned from SpeedRunsLive.</p>
        <p>If you happen to leave during a proofcall or become inactive for a longer period of time, you will also receive a
            disqualification, and if we feel necessary, a ban from SpeedRunsLive.</p>
        ...
        <p>A season is a 3 month, ranked competition of races. Season 1 ran from April 12, 2013 to September 8, 2013. Streaming
            is required for Seasons goals, regardless of whether you're playing on console or emulator. To learn more about
            seasons, <Link to="/races/#!/seasons/1">click here</Link>.</p>
        ...
        <p>It doesn't. We use real time for everything, so the only thing that determines the winner is the order in which they
            finish in real time. In-game timers tend to be easily abused, and oftentimes do not count many things such as
            pause screens, transitions, or lag.</p>
        ...
        <p>Not allowed when the official stock controller doesn't have this function. Turbo is unfair to those who don't have
            a third party controller with a turbo function.</p>
        <h3>Glitch-abuse</h3>
        <p>Allowed, but sometimes limited. One way to limit glitch abuse is to have certain rulesets or categories that have
            restrictions on what you can and cannot do. Generally glitches will be allowed, however. If you don't really
            understand why, then consider this: The game merely executes the code in the way it was programmed to do. The
            game is the law. If you start trying to get at "developer intentions," then you start a game of guesswork trying
            to figure out what exactly was intended or not.</p>
        <h3>Cheat-codes</h3>
        <p>If via a 3rd party device such as a gameshark or game genie, this is not allowed. These generally water down the
            game and remove the element of skill, which is against the whole point of competition. Using in-game cheats,
            if specifically mentioned in the goal, is allowed.</p>
        <h3>Savestates</h3>
        ...
        <p>In some games, if you save and reload it will warp you to a certain point faster than if you travelled there naturally.
            Typically allowed, but this is a community-driven rule, and if it becomes more popular to disallow saves for
            a certain game, then so be it.</p>
        <h3>Scripts</h3>
        ...
        <p>Type <code>.setstream twitch/<em>name</em></code> to link your stream to SRL, if you have not done so already. Your
            stream will appear on the Streams page of the site after 1 hour of racing. You may not appear immediately, as
            our system checks at specific times throughout the day.
        </p>
        <p>
        ...
        <p>If you are found streaming non-speedrun content to the front page, you will be flagged and automatically hidden.
            Depending on quantity of recent offenses, you will be temporarily blacklisted as follows:</p>
        <ul>
        ...
        <p>
            ...
            appear at the top of your list. You may also hide streamers and games you do not want to see, so that they appear
            in a hidden menu at the bottom of your list.
        </p>
        ...
        <ol>
            ...
            <li>All donation links presented on stream or marathon related websites should go directly to the charity's PayPal
                or another proven payment method. No proceeds should go towards personal causes.</li>
        </ol>
        <p>Additional notes:</p>
        <ul>
            <li>If you'd like to use PayPal but your charity does not accept it, you should contact them ahead of time to see
                if they are able to set up a PayPal address for you in their domain name.</li>
            ...
            <li>Users are free to host marathons for whatever cause they wish on their own SRL nick under their own Twitch name,
                be it for charity, personal, etc. Exploiting a loophole by using one’s racing nick and doing <code>.setstream</code>                to a marathon Twitch channel is not acceptable, and may result in your stream being temporarily blacklisted
                from the front page.</li>
            <li>After the marathon concludes, the stream will be un-whitelisted. If another marathon happens on the channel in
                the future, please re-submit the request.</li>
        </ul>
        <p>If you would like your marathon to be whitelisted, please PM a moderator, and/or send an e-mail to the SRL e-mail
            address at <a href="mailto:staff@speedrunslive.com">staff@speedrunslive.com</a>.</p>
        ...
        <p>Also, if you're still feeling unsure of how everything works here, feel free to lurk in <strong>#speedrunslive</strong>,
            check out a few races, linger in race channels, and ask questions (even if the people tell you to look at the
            FAQ again, which isn't a bad idea either).</p>
    </div>
</div>
    );
}
